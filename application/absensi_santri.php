            ...
            <div class="col-xs-12">  
              <div class="box">
                <div class="box-header">
                  <h3 class="box-title"><?php if (isset($_GET[kelas]) AND isset($_GET[tahun])){ echo "Absensi santri"; }else{ echo "Absensi santri Pada Tahun ".date('Y'); } ?></h3>
                  <form style='margin-right:5px; margin-top:0px' class='pull-right' action='' method='GET'>
                    <input type="hidden" name='view' value='absensantri'>
                ...
                <div class="box-body">
                  ...
                  <?php
                    if (isset($_GET[kelas]) AND isset($_GET[tahun])){
                      $tampil = mysqli_query($koneksi,"SELECT a.*, e.nama_kelas, b.namamatapelajaran, b.kode_pelajaran, b.kode_kurikulum, c.nama_Ustadz, d.nama_ruangan FROM jadwal_pelajaran a 
                                            JOIN mata_pelajaran b ON a.kode_pelajaran=b.kode_pelajaran
                                              JOIN Ustadz c ON a.nip=c.nip 
                    ...
                    echo "<tr><td>$no</td>
                        ...
                        echo "<td style='width:70px !important'><center>
                                <a class='btn btn-success btn-xs' title='Tampil List Absensi' href='index.php?view=absensantri&act=tampilabsen&id=$r[kode_kelas]&kd=$r[kode_pelajaran]&jdwl=$r[kodejdwl]'><span class='glyphicon glyphicon-th'></span> Tampilkan</a>
                              </center></td>";
                              }
                            echo "</tr>";
                      $no++;
    ...
    echo "<div class='col-md-12'>
              <div class='box box-info'>
                <div class='box-header with-border'>
                  <h3 class='box-title'>Data Absensi santri Pada : <b style='color:red'>".tgl_indo("$filtertgl")."</b></h3>
                </div>
              <div class='box-body'>
              ...
              <form action='index.php?view=absensantri&act=tampilabsen&id=$_GET[id]&kd=$_GET[kd]' method='POST' class='col-md-7 pull-right' style='margin-bottom:5px'>
              ...
              <input type='hidden' name='jdwl' value='$_GET[jdwl]'>
                <div class='col-md-12'>
                  <table class='table table-condensed table-bordered table-striped'>
                    ...
                    $no = 1;
                    $tampil = mysqli_query($koneksi,"SELECT * FROM santri a JOIN jenis_kelamin b ON a.id_jenis_kelamin=b.id_jenis_kelamin where a.kode_kelas='$_GET[id]' ORDER BY a.id_santri");
                    while($r=mysqli_fetch_array($tampil)){
                    if ($_GET[gettgl]){
                    ...
                    $a = mysqli_fetch_array(mysqli_query($koneksi,"SELECT * FROM absensi_santri where kodejdwl='$_GET[jdwl]' AND tanggal='$sekarangabsen' AND nisn='$r[nisn]'"));
              ...
              echo "</form>
                ...
                if (isset($_POST[simpann])){
                    ...
                    for ($i=1; $i <= $jml_data; $i++){
                      $cek = mysqli_query($koneksi,"SELECT * FROM absensi_santri where kodejdwl='$_POST[jdwl]' AND nisn='".$nisn[$i]."' AND tanggal='".$e."-".$f."-".$g."'");
                      $total = mysqli_num_rows($cek);
                      if ($total >= 1){
                        mysqli_query($koneksi,"UPDATE absensi_santri SET kode_kehadiran = '".$a[$i]."' where nisn='".$nisn[$i]."' AND kodejdwl='$_POST[jdwl]'");
                        $cs = mysqli_fetch_array(mysqli_query($koneksi,"SELECT * FROM santri a JOIN kelas b ON a.kode_kelas=b.kode_kelas where a.nisn='".$nisn[$i]."'"));
                        if ($a[$i] != 'H'){
                        ...
                        mysqli_query($koneksi,"INSERT INTO absensi_santri VALUES('','$_POST[jdwl]','".$nisn[$i]."','".$a[$i]."','".$e."-".$f."-".$g."','".date('Y-m-d H:i:s')."')");
                        $cs = mysqli_fetch_array(mysqli_query($koneksi,"SELECT * FROM santri a JOIN kelas b ON a.kode_kelas=b.kode_kelas where a.nisn='".$nisn[$i]."'"));
                        if ($a[$i] != 'H'){
                          if ($a[$i] == 'A'){ $statush = 'Alpa'; }elseif ($a[$i] == 'S'){ $statush = 'Sakit'; }elseif ($a[$i] == 'I'){ $statush = 'Izin'; }
                          $isi_pesan = "Diberitahukan kepada Yth Bpk/Ibk, Bahwa anak anda $cs[nama], $cs[nama_kelas] absensi Hari ini Tanggal $g-$f-$e : $statush";
                    ...
                    echo "<script>document.location='index.php?view=absensantri&act=tampilabsen&id=".$_POST[kelas]."&kd=".$_POST[pelajaran]."&jdwl=".$_POST[jdwl]."&gettgl=".$e."-".$f."-".$g."';</script>";
                }
}elseif($_GET[act]=='detailabsenUstadz'){ ?>
            <div class="col-xs-12">  
              <div class="box">
                <div class="box-header">
                  <h3 class="box-title"><?php if (isset($_GET[tahun])){ echo "Absensi santri"; }else{ echo "Absensi santri Pada ".date('Y'); } ?></h3>
                  <form style='margin-right:5px; margin-top:0px' class='pull-right' action='' method='GET'>
                    <input type="hidden" name='view' value='absensantri'>
                    <input type="hidden" name='act' value='detailabsenUstadz'>
                    <select name='tahun' style='padding:4px'>
                        <?php 
                            echo "<option value=''>- Pilih Tahun Akademik -</option>";
                            $tahun = mysqli_query($koneksi,"SELECT * FROM tahun_akademik");
                ...
                <div class="box-body">
                  ...
                  <?php
                    if (isset($_GET[tahun])){
                      $tampil = mysqli_query($koneksi,"SELECT a.*, e.nama_kelas, b.namamatapelajaran, b.kode_pelajaran, c.nama_Ustadz, d.nama_ruangan FROM jadwal_pelajaran a 
                                            JOIN mata_pelajaran b ON a.kode_pelajaran=b.kode_pelajaran
                                              JOIN Ustadz c ON a.nip=c.nip 
                                                JOIN ruangan d ON a.kode_ruangan=d.kode_ruangan
                                                  JOIN kelas e ON a.kode_kelas=e.kode_kelas 
                    ...
                    echo "<tr><td>$no</td>
                              ...
                              <td>$r[nama_ruangan]</td>
                              <td>$r[id_tahun_akademik]</td>
                              <td><a class='btn btn-success btn-xs' title='Tampil List Absensi' href='index.php?view=absensantri&act=tampilabsen&id=$r[kode_kelas]&kd=$r[kode_pelajaran]'><span class='glyphicon glyphicon-th'></span> Tampilk Absensi</a></td>
                          </tr>";
                      $no++;
                      }
                  ?>
...
<?php
}elseif($_GET[act]=='detailabsensantri'){ 
    echo "<div class='col-xs-12'>  
              <div class='box'>
                <div class='box-header'>
                  <h3 class='box-title'>Data Absensi santri untuk Mata Pelajaran yang di Ampu</h3>
                </div>
                <div class='box-body'>
                ...
                <table class='table table-bordered table-striped'>
                    ...
                    echo "<tr><td>$no</td>
                              ...
                              <td style='width:70px !important'><center>
                                <a class='btn btn-success btn-xs' title='Tampil List Absensi' href='index.php?view=absensantri&act=tampilabsen&id=$r[kodekelas]&kd=$r[kodepelajaran]'><span class='glyphicon glyphicon-th'></span> Tampilkan Absensi</a>
                              </center></td>";
                            echo "</tr>";
                      $no++;
                      }
